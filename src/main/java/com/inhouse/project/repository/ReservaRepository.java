...
public interface ReservaRepository extends JpaRepository<Reserva, Long> {
  ...
  @Query(value = """
            SELECT g::date AS dia_reservado
            FROM reservas r
            CROSS JOIN LATERAL generate_series(
                r.data_checkin,
                r.data_checkout - interval '1 day',
                interval '1 day'
            ) g
            WHERE r.casa_id = :casaId
            """, nativeQuery = true)
    List<java.sql.Date> findDiasReservadosByCasaIdSql(@Param("casaId") Long casaId);
